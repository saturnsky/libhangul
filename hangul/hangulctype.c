...
{
    c = hangul_vkeycode_to_keycode(c);
...
/**
 * @ingroup hangulctype
 * @brief 가상 키코드를 일반 키코드로 변환
 * @param c UCS4 코드 값
 * @return @a c가 가상 키코드면 일반 키코드로 변환한 값, 그 외에는 @a c
 *
 * 이 함수는 @a c로 주어진 UCS4 코드가 가상 키코드인지 확인하고 일반 키코드로 변환한다.
 */
ucschar
hangul_vkeycode_to_keycode(ucschar c)
{
    if (c >= 0xe000 && c <= 0xe0ff)
        return c - 0xe000 + 0x1100;
    else
        return c;
}
...
hangul_jamo_to_syllable(ucschar choseong, ucschar jungseong, ucschar jongseong)
{
    ...
    choseong = hangul_vkeycode_to_keycode(choseong);
    jungseong = hangul_vkeycode_to_keycode(jungseong);
    jongseong = hangul_vkeycode_to_keycode(jongseong);
...
choseong_compress(ucschar a, ucschar b)
{
    ...
    a = hangul_vkeycode_to_keycode(a);
    b = hangul_vkeycode_to_keycode(b);
